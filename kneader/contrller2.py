...
class KneaderController:
    ...
    async def _process_workorder(self, initial_barcode: Optional[str] = None):
        try:
            ...
            #await self.logger.log("INFO", "Starting prescanning phase",
                                #data=self.get_full_status(), is_event=True)
            
            # Perform prescanning - BLOCKS until all items are scanned
            #prescan_success = await self.pre_scanning(self.workorder)
            ...
            # Only proceed to actual mixing if prescanning successful
            # After prescanning
            self.process_state = "WAITING_FOR_ITEMS"
            await self.logger.log("INFO", "Prescanning completed, Waiting for user confirmation to start mixing.",  
                                    data=self.get_full_status(), is_event=True)

            cmd, future = await self.hmi_cmd_queue.get()

            #cmd, future = await self.hmi_cmd_queue.get()
            """if cmd["command"] == "confirm_start":
                if future:
                    future.set_result({"status": "success", "message": "Starting actual process"})
                self.process_state = "WAITING_FOR_ITEMS"
            else:
                if future:
                    future.set_result({"status": "fail", "message": "Expected confirm_start command"})
                return"""
            self.process_state = "WAITING_FOR_ITEMS"
            await self.logger.log("INFO", "User confirmed, starting actual process", 
                    data=self.get_full_status(), is_event=True)
                ...
                if self.gateway.is_connected:
                    ...
                    if self.process_state == "MIXING" and self.lid_open:
                        ...
                        if self.work_order_task and not self.work_order_task.done():
                            ...
                            # 🔹 NEW: Log every 5 seconds while paused/aborted
                    if self.process_state == "ABORTED":
                        now = time.time()
                        if now - last_aborted_log >= 2:
                            await self.logger.log("INFO","Workorder is paused (ABORTED state) - waiting for operator action",data=self.get_full_status(),is_event=False)
        ...
        try:
            while True:
                ...
                if command == "abort":
                    ...
                    if self.process_state in ("PRESCAN_COMPLETE", "PRESCANNING"):
                        ...
                        if self.work_order_task and not self.work_order_task.done():
                            ...
                            # Enqueue into run() loop as load_and_start_workorder

                            future = asyncio.get_running_loop().create_future()

                            await self.hmi_cmd_queue.put((

                                {"command": "load_and_start_workorder", "data": self.workorder},
                ...
                # In the hmi_client_handler method, add these cases:
                    ...
                    if self.process_state == "PRESCANNING":
                        # Handle prescan directly instead of putting in queue
                        future = asyncio.get_running_loop().create_future()
                        # Create a mock command structure that pre_scanning expects
                        mock_cmd = {"command": "prescan_item", "data": message.get("data", {})}
                        # Process the scan directly in the prescanning context
                        asyncio.create_task(self._process_prescan_item(mock_cmd, future))
                        response = await asyncio.wait_for(future, timeout=15.0)
                    else:
                        # For non-prescanning states, use the normal queue approach
                        future = asyncio.get_running_loop().create_future()
    ...
    async def run(self):
        ...
        # Clear old workorder info
                        ...
                        """if self.process_state not in ["ERROR", "PROCESS_COMPLETE"]:
                            self.process_state = "IDLE"""
